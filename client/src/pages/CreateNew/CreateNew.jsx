...
const CreateNew = () => {
    ...
    function handleSubmit(e){
        e.preventDefault();
        if(!objeto.name || objeto.name.length > 20 || !nombreValido.test(objeto.name) ) return alert('Solo puede llevar letras y su largo debe ser menor a 20')
        if(!objeto.hp || objeto.hp <= 0 || objeto.hp > 200) return alert('Debe ser mayor a 0 y menor a 200')
        if(!objeto.attack || objeto.attack <= 0 || objeto.attack > 200) return alert('Debe ser mayor a 0 y menor a 200')
        if(!objeto.defense || objeto.defense <= 0 || objeto.defense > 200) return alert('Debe ser mayor a 0 y menor a 200')
        if(!objeto.speed || objeto.speed <= 0 || objeto.speed > 300) return alert('Debe ser mayor a 0 y menor a 300')
        if(!objeto.height ||objeto.height <= 0 || objeto.height > 100) return alert('Debe ser mayor a 0 y menor a 100')
        if(!objeto.weight || objeto.weight <= 0 || objeto.weight > 1000) return alert('Debe ser mayor a 0 y menor a 1000')
        if(objeto.types.length === 0 || objeto.types.length > 2) return alert('Solo pueden seleccionarse máximo 2 tipos')
        ...
        <div className={styles.container}>
            ...
            <form onSubmit={handleSubmit} className={styles.todo}>
                ...
                <div className={styles.center}>
                    <button type= 'submit' className={styles.btnSubmit}>¡Agregar Pokemon!</button>
                </div>
            </form>
        </div>
    )
